...
class RSSM(nn.Module):
    def __init__(
        ...
        jmp_inp_layers = []
        if self._discrete:
            jmp_inp_dim = self._stoch * self._discrete
        else:
            jmp_inp_dim = self._stoch
        jmp_inp_layers.append(nn.Linear(jmp_inp_dim, self._hidden, bias=False))
        if norm:
            jmp_inp_layers.append(nn.LayerNorm(self._hidden, eps=1e-03))
        jmp_inp_layers.append(act())
        self._jmp_in_layers = nn.Sequential(*jmp_inp_layers)
        self._jmp_in_layers.apply(tools.weight_init)
        self._cell_jmp = GRUCell(self._hidden, self._deter, norm=norm)
        self._cell_jmp.apply(tools.weight_init)
        '''
        ...
        jmp_out_layers = []
        inp_dim = self._deter
        jmp_out_layers.append(nn.Linear(inp_dim, self._hidden, bias=False))
        if norm:
            jmp_out_layers.append(nn.LayerNorm(self._hidden, eps=1e-03))
        jmp_out_layers.append(act())
        self._jmp_out_layers = nn.Sequential(*jmp_out_layers)
        self._jmp_out_layers.apply(tools.weight_init)
        '''
        ...
        if self._discrete:
            ...
            # self._jmp_stat_layer = nn.Linear(self._hidden, self._stoch * self._discrete)
            # self._jmp_stat_layer.apply(tools.uniform_weight_init(1.0))
            self._obs_stat_layer = nn.Linear(self._hidden, self._stoch * self._discrete)
            self._obs_stat_layer.apply(tools.uniform_weight_init(1.0))
        else:
            self._imgs_stat_layer = nn.Linear(self._hidden, 2 * self._stoch)
            self._imgs_stat_layer.apply(tools.uniform_weight_init(1.0))
            # self._jmp_stat_layer = nn.Linear(self._hidden, 2 * self._stoch)
            # self._jmp_stat_layer.apply(tools.uniform_weight_init(1.0))
    ...
    def initial(self, batch_size):
        # print("RSSM batch_size", batch_size)
    ...
    def observe(self, embed, action, is_first, state=None):
        ...
        #     post[k].register_hook(lambda grad: print(f"Gradient at post[{k}]:", grad))
    ...
    '''
    def obs_step_zoomed(self, rely_state, prev_action, embed_zoomed, is_first_zoomed, sample=True):
        if rely_state == None or torch.sum(is_first_zoomed) == len(is_first_zoomed):
            rely_state = self.initial(len(is_first_zoomed))
            prev_action = torch.zeros((len(is_first_zoomed), self._num_actions)).to(self._device)

        elif torch.sum(is_first_zoomed) > 0:
            is_first_zoomed = is_first_zoomed[:, None]
            prev_action *= 1.0 - is_first_zoomed
            init_state = self.initial(len(is_first_zoomed))
            for key, val in rely_state.items():
                is_first_zoomed_r = torch.reshape(
                    is_first_zoomed,
                    is_first_zoomed.shape + (1,) * (len(val.shape) - len(is_first_zoomed.shape)),
                )
                rely_state[key] = (
                    val * (1.0 - is_first_zoomed_r) + init_state[key] * is_first_zoomed_r
                )

        prior = self.jmp_step(rely_state)
        x = torch.cat([prior["deter"], embed_zoomed], -1)
        # (batch_size, prior_deter + embed) -> (batch_size, hidden)
        x = self._obs_out_layers(x)
        # (batch_size, hidden) -> (batch_size, stoch, discrete_num)
        stats = self._suff_stats_layer("obs", x)
    ...
    '''
    def jmp_step(self, prev_state, sample=True):
        prev_stoch = prev_state["stoch"]
        if self._discrete:
            shape = list(prev_stoch.shape[:-2]) + [self._stoch * self._discrete]
            prev_stoch = prev_stoch.reshape(shape)

        x = self._jmp_in_layers(prev_stoch)

        for _ in range(self._rec_depth):  # rec depth is not correctly implemented
            deter = prev_state["deter"]
            x, deter = self._cell_jmp(x, [deter])
            deter = deter[0]  # Keras wraps the state in a list.
        x = self._jmp_out_layers(x)
        stats = self._suff_stats_layer("jmp", x)
        if sample:
            stoch = self.get_dist(stats).sample()
        else:
            stoch = self.get_dist(stats).mode()
    ...
    '''
    def get_stoch_jmp(self, deter):
        x = self._jmp_out_layers(deter)
        stats = self._suff_stats_layer("jmp", x)
        dist = self.get_dist(stats)
        return dist.mode()
    '''
...
class MultiEncoder(nn.Module):
    ...
    def forward(self, obs):
        outputs = []
        if self.cnn_shapes:
            # print("==========MultiEncoder cnn_shapes==========")
            # for k in self.cnn_shapes:
            #     print(k, obs[k].shape)
            inputs = torch.cat([obs[k] for k in self.cnn_shapes], -1)
            outputs.append(self._cnn(inputs))
        if self.mlp_shapes:
            # print("==========MultiEncoder mlp_shapes==========")
