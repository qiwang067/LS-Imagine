...
class Logger:
    def __init__(self, config, logdir, step):
        ...
        #
        wandb.init(project="LongDream", dir=str(logdir), config = config.__dict__, name = str(logdir), save_code=True)
...
def calculate_accumulated_reward(rewards, intrinsics, gamma, half_indicator):
    if len(rewards) == 0:
        return 0
...
def simulate( #
    ...
    gamma,
    is_eval=False,
    limit=None, # config.dataset_size # 1000000
    steps=0, # config.eval_every 10000.0
    episodes=0,
    state=None,
    is_training=False,
):

    half_indicator = is_gaussian
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if steps:
            print(f"==================== Simulate step: {step}, total step: {steps} ===================")
        else:
            episodes = 0
            print(f"==================== Simulate episode: {episode}, total episode: {episodes} ===================")
        ...
        if done.any():
            ...
            for index, result in zip(indices, results):
                ...
                if is_zoomed:
                    # length = len(cache[envs[index].id]["reward"]) 
                    # print(f"length: {length}")
                    current_step = 0
                    if t["is_zoomed"] == True:
                        # print(f"env_id: {envs[index].id}, current_step: {zoomed_step}")
        ...
        success = list([p.get("success", False) for p in information])
        episode += int(done.sum())
        length += 1
        step += len(envs)
        length *= 1 - done

        # zoom_in_threshold_list = list([p.get("zoom_in_threshold", 1.0) for p in info])
        # zoom_in_threshold = sum(zoom_in_threshold_list) / len(zoom_in_threshold_list)
        # agent._wm.zoom_in_threshold = zoom_in_threshold
        # if is_training:
            # agent._wm.zoom_in_threshold = zoom_in_threshold
            # print(agent._wm.zoom_in_threshold)
            # print(zoom_in_threshold)
        ...
        for tmp_index in range(len(results)):
            ...
            transition["first_success_step"] = info.get("first_success_step", 0)
            # transition["zoom_in_threshold"] = info.get("zoom_in_threshold", 1.0)
            ...
            if is_zoomed:
                ...
                if transition["is_zoomed"] == True and not d:
                    # print(f"env_id: {env.id}, current_step: {current_step}")
                ...
                if len(tmp_list) > 0:
                    ...
                    for ss in tmp_list:
                        ...
                        cache[env.id]["accumulated_reward"][ss] = calculate_accumulated_reward(cache[env.id]["reward"][ss+1:current_step], cache[env.id]["intrinsic"][ss+1:current_step], gamma, half_indicator)
                        cache[env.id]["is_calculated"][ss] = True

                        print("is_zoomed:", cache[env.id]["is_zoomed"][ss])
                        print("jump:", cache[env.id]["jump"][ss])
                        print("jumping_steps:", cache[env.id]["jumping_steps"][ss])
                        print("accumulated_reward:", cache[env.id]["accumulated_reward"][ss])
        ...
        if done.any():
            ...
            for i in indices:
                ...
                #
                #
                if is_zoomed and (not is_eval) and (not information[i].get("real_done", False)):
                    continue
                ...
                #     first_success_step = length
                # threshold = cache[envs[i].id]["zoom_in_threshold"][-1]
                # threshold = float(np.array(cache[envs[i].id]["zoom_in_threshold"]).mean())
                video = cache[envs[i].id]["image"]
                attack_num = 0
                acs = cache[envs[i].id]["action"]
                for ac in acs:
                    # print(ac)
                    if ac[1] == 1:
                        attack_num += 1
                ...
                #
                # print("==========checking zoom_in_threshold==========")
                ...
                if not is_eval:
                    ...
                    # logger.scalar(f"train_zoom_in_threshold", threshold)
                    logger.scalar(f"attack_num", attack_num)
                    ...
                    eval_attack_num.append(attack_num)
                    ...
                    # threshold = sum(eval_thresholds) / len(eval_thresholds)
                    attack_num = sum(eval_attack_num) / len(eval_attack_num)
                    ...
                    if len(eval_scores) >= episodes and not eval_done:
                        ...
                        # logger.scalar(f"eval_zoom_in_threshold", threshold)
                        logger.scalar(f"eval_attack_num", attack_num)
                        logger.write(step=logger.step)
                        eval_done = True
    if is_eval:
...
def lambda_return_for_longdream(reward, value, gamma, end, jumping_steps, accumulated_reward, bootstrap, lambda_, axis):
...
def static_scan(fn, inputs, start):
    ...
    last = start
    indices = range(inputs[0].shape[0])
    # print("info from static_scan: len(indices)=", len(indices))
    flag = True
    for index in indices:
        inp = lambda x: (_input[x] for _input in inputs)
        last = fn(last, *inp(index))
        ...
        if flag:
            if type(last) == type({}):
                # print("info from static_scan: last is a dict")
                outputs = {
                    key: value.clone().unsqueeze(0) for key, value in last.items()
                }
            else:
                # print("info from static_scan: last is not a dict")
                outputs = []
                for _last in last:
                    if type(_last) == type({}):
                        outputs.append(
